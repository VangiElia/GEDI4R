...
#'   rbind. Ignored if \code{length(gediL4_path)==1}
#' @param catch Logical: if TRUE (default) file that would throw an error don't
...
#' @seealso \code{\link[=l4_get]{l4_get}}, \code{\link[=l4_process]{l4_process}}
#' @examples
#'
...
#' #merge all file
#' l4_data <- l4_getmulti(l4,ncore = parallel::detectCores()-1,merge = T)
#' #return individual files
#' l4_data <- l4_getmulti(l4,ncore = parallel::detectCores()-1,merge = F)
#' #add user deifined dataset
#' col_name <- "agbd_pi_lower"
#' l4_data <- l4_getmulti(l4,add_col = col_name,ncore =
#' parallel::detectCores()-1,merge = T)
#' @export

l4_getmulti <- function(gediL4_path,just_colnames=F,add_col=NULL,tct=NULL,agbd_rm=NULL,ncore=parallel::detectCores()-1,merge=T,catch=T,source=F){
  ...
  if(length(gediL4_path)==1){
    warning("gediL4_path has lenght==1,l4_get will be used in  single thread mode")
    return(l4_get(gediL4_path,just_colnames = just_colnames,add_col=add_col,tct=tct,agbd_rm=agbd_rm))
    }
  ...
  readL4 <- function(path,add_col=add_col,tct=tct,agbd_rm=agbd_rm){
  ...
  if(catch){
    test_function <- function(gediL4_path,add_col=add_col,tct=tct,agbd_rm=agbd_rm){
      return(tryCatch(readL4(gediL4_path,add_col=add_col,tct=tct,agbd_rm=agbd_rm),error=function(e)NULL))
    }
  }else{
  ...
  l4_list <- snowfall::sfLapply(gediL4_path,test_function,add_col=add_col,tct=tct,agbd_rm=agbd_rm)
  snowfall::sfStop()
  ...
  if(merge) {
    if (!source) {
      ...
      n <- unlist(lapply(l4_list, nrow))
      l4_list <- do.call(rbind, l4_list)
      if (all(!failed)) {
        l4_list$source <- rep(gediL4_path, times = n)
      } else{
        l4_list$source <- rep(gediL4_path[-failed], times = n)
      }
    }
  }
  message("Finish!")
