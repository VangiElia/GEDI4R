...
#'   and \code{append=FALSE} in \code{l4_convert}.
...
l4_process <- function(gediL4_path,nfile=NULL,clip=NULL,epsg=NULL,prefix="chunk",outdir=NULL,ext=NULL,parallel=T,ncore=NULL,add_col=NULL,tct=NULL){
  ...
  process <- function(index){
    ...
    clipped <- suppressWarnings(suppressMessages(l4_clip(tmp,clip=clip,usegeometry = F)))
    clipped <- suppressWarnings(suppressMessages(l4_clip(clipped,clip=clip,usegeometry = T)))
